...
const checkValues = (obj, value) => {
  for (let key in obj) {
    if (obj.hasOwnProperty(key) && obj[key] === value) {
      return true; // If value is found in the object, return true
    }
  }
  return false; // If value is not found in the object, return false
};
...
const hasChildrenValues = (arr, character) => {
  const characterInfo = arr.find(char => char.name === character);
  if (characterInfo) {
    const children = Object.values(characterInfo.children || []);
